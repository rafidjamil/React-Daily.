...
const Login = () => {
  ...
  // 1. Check if user is already logged in (Redirection logic)
  useEffect(() => {
    const loggedInUser = JSON.parse(localStorage.getItem("loggedInUser"));
    if (loggedInUser) {
      // Agar user pehle se login hai, to use login page nahi dikhana
      navigate('/user'); 
    }

    // Dummy users data initialize karna
    const users = [
  ...
  const handleSubmit = (e) => {
    ...
    if (user) {
      ...
      navigate('/user'); // Login ke baad profile par bhejna
    } else {
